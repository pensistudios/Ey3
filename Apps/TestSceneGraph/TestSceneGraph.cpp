...
bool boxLeft = false;
bool boxRight = false;
bool boxRotate = false;
bool boxRotateBack = false;
bool lboxLeft = false;
bool lboxRight = false;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	SceneNode* node = new SceneNode(2);
	std::ifstream boxFile("Box.txt");
	std::ifstream boxFile2("Box2.txt");
	Ey3::SimpleGeometry* boxGeom1 = Ey3::SimpleGeometry::createFromStream(boxFile);
	Ey3::SimpleGeometry* boxGeom2 = Ey3::SimpleGeometry::createFromStream(boxFile2);

	boxGeom1->local.setTranslate(Vector3(2.0f,0.0f,0.0f));
	boxGeom2->local.setTranslate(Vector3(-2.0f,0.0f,0.0f));

	boxGeom1->updateWorldTransform();
	boxGeom2->updateWorldTransform();

	node->setChild(0, boxGeom1);
	node->setChild(1, boxGeom2);
		
	node->local.setUniformScale(0.5f);
	node->updateWorldTransform();

	node->local.setUniformScale(1.0f);
	node->local.setTranslate(Vector3::ZERO);
	node->updateWorldTransform();
	boxGeom1->local.setTranslate(Vector3::ZERO);
	boxGeom2->local.setTranslate(Vector3::ZERO);
	boxGeom1->updateWorldTransform();
	boxGeom2->updateWorldTransform();
    ...
    while (TRUE)
    {
		...
		if(boxRight)
		{
			node->local.setTranslate(Vector3(-0.001f,0.0f,0.0f));
			node->updateWorldTransform();
		}
		...
		if(boxRotate)
		{
			rot.fromAxisAngle(Vector3::UNIT_X,rotSpeed);

			node->local.setRotate(rot);
			node->updateWorldTransform();
		}	
		...
		if(lboxLeft)
		{
			boxGeom1->local.setTranslate(Vector3(0.001f,0.0f,0.0f));
			boxGeom1->updateWorldTransform();
		}
		...
		if(!boxRotate && !boxRotateBack)
		{
			node->local.setRotate(Matrix3::IDENTITY);
		}
