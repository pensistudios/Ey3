...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	std::ifstream xzFile("XZ.txt");
	SimpleGeometry* xzGeom = SimpleGeometry::createFromStream(xzFile);
	std::ifstream yzFile("YZ.txt");
	SimpleGeometry* yzGeom = SimpleGeometry::createFromStream(yzFile);
	Ey3::SceneNode* node = new SceneNode(3);
	node->setChild(0,xzGeom);
	node->setChild(1,xyGeom);
	node->setChild(2,yzGeom);
	...
	Ey3::RenderObject* node = new Ey3::SceneNode(2);
	((Ey3::SceneNode*)node)->setChild(0, tankGeom);
	((Ey3::SceneNode*)node)->setChild(1, turret);

	node->local.setUniformScale(0.25f);
	Matrix3 q;
	q.setColumn(Vector3(1.0f,0.0f,0.0f),0);
	q.setColumn(Vector3(0.0f,cosf(Math::PI/2.0f),1.0f),1);
	q.setColumn(Vector3(0.0f,-1.0f,cosf(Math::PI/2.0f)),2);
	node->local.setRotate(q);
	node->updateWorldTransform();
	*/
	/*
	SceneNode* node = new SceneNode(1);
	std::ifstream boxFile("Box.txt");
	Ey3::SimpleGeometry* boxGeom = Ey3::SimpleGeometry::createFromStream(boxFile);

	node->setChild(0, boxGeom);	
	
	//node->local.setUniformScale(1.0f);
	//node->local.setTranslate(Vector3(0.0f,0.0f,-4.0));
	node->updateWorldTransform();
		
	//node->local.setTranslate(Vector3::ZERO);
*/
    ...
    while (TRUE)
    {
		...
		/*rot.setColumn(Vector3(cosf(rad),0,-sinf(rad)),0);
		rot.setColumn(Vector3(0.0f,1.0f,0.0f),1);
		rot.setColumn(Vector3(sinf(rad),0,cosf(rad)),2);
		...
		{     
			...
			axis = Ey3::crossProduct(dir,cam->getWorldDVector());
			
			rad+=0.001f;

			rot = rot.fromAxisAngle(axis,rad);

			cam->setAxes(rot);

			/*char buf[400];
			sprintf(buf,"Dir: %f %f %f DVec: %f %f %f Axis: %f %f %f Rad: %f",
				dir.x,dir.y,dir.z,
				cam->getWorldDVector().x,cam->getWorldDVector().y,cam->getWorldDVector().z,
